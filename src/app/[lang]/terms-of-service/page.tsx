...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return { title: dict['terms_of_service']?.meta_title || 'terms-of-service', description: dict['terms_of_service']?.meta_desc || 'terms-of-service tool' };
}

export default async function ToolPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params; const dict = await getDictionary(lang);
  const toolDict = dict['terms_of_service'] || { h1: 'terms-of-service Utility', meta_desc: 'Online web tool utility', placeholder: 'Enter inputs...', btn_format: 'Execute' };
  return (
