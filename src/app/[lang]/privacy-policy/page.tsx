...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return { title: dict['privacy_policy']?.meta_title || 'privacy-policy', description: dict['privacy_policy']?.meta_desc || 'privacy-policy tool' };
}

export default async function ToolPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params; const dict = await getDictionary(lang);
  const toolDict = dict['privacy_policy'] || { h1: 'privacy-policy Utility', meta_desc: 'Online web tool utility', placeholder: 'Enter inputs...', btn_format: 'Execute' };
  return (
